...
class MyModel(KerasModel):
  ...
  def GetDataSet(self, filenames, transform):
    ...
    if not transform == '-':
      for t in self.hparam[transform]:
        ...
        if t['name'] == 'OneHot':
          dataset = dataset.map(replace_OneHot, num_parallel_calls=at)
 ...
 #   self.dft.to_csv(submissionCSV, header=True, index=False)
  ...
  def Compile(self, model, loss = 'mse', metric=['mse'], loss_weight=None):
    # compile the model
  ...
  def Train(self, ds, dsv):
    ...
    # elif arch == 'LGB':
    #  model.Train()
    if not (self.hparam['arch'] == 'AE' or self.hparam['arch'] == 'MC' or self.hparam['arch'] == 'MCS' or self.hparam['arch'] == 'NR'):
...
with strategy.scope():
  ...
  nm = MyModel(args.model, hyperParam)
...
if False:
  ...
  print(ex[0][1], ex[0][0][1], ex[0][0][2])
