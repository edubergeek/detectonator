...
"""
Implement a neural network model using keras.
...
class KerasModel(BaseModel):
  ...
  def Fit(self):
    ...
    self.model.fit(self.xTrain, self.yTrain, validation_data=(self.xValid, self.yValid), epochs=self.epochs, batch_size=self.batchSize, callbacks=self.callbacks, verbose=0, shuffle=1)
    self.isTrained = True
  ...
  def Predict(self, X):
    pred = self.model.predict(X, batch_size=self.batchSize)
    return pred
